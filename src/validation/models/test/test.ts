...
const validateCreateTest = [
  // 验证 name
  body('name').notEmpty().withMessage('姓名不能为空').isLength({ min: 2, max: 50 }).withMessage('姓名长度在2-50之间').trim(),

  // 验证 email
  body('email').notEmpty().withMessage('邮箱不能为空').isEmail().withMessage('邮箱格式不正确').normalizeEmail(), // 自动转为小写
